...
func generate_chunk(position):
	...
	for x in range(width):
		for y in range(height):
			...
			if alt < 2:
				set_cell(0, Vector2i(tile_pos.x-width/2 + x, tile_pos.y-height/2 + y), 1, Vector2(0, round((temp+10)/5)))
			else:
				set_cell(0, Vector2i(tile_pos.x-width/2 + x, tile_pos.y-height/2 + y), 0, Vector2(0, round((temp+10)/5)))
